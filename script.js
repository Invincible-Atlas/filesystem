// the JSON representation of the file tree that will be used to generate the page
const fstree = [{"name":"file1.mp3"},{"name":"file2.zip"},{"name":"folder1","contents":[{"name":"file3.file"},{"name":"file4.file"}]}]
...
function generate(tree,isFolder){
    let fsContainer = document.createElement("div");
    // fsContainer.classList = "fsContainer";
    for(i in tree){
        // let container = document.createElement("div");
        if(tree[i].contents){
            let container = document.createElement("div");
            container.classList="folder";
            let ico = document.createElement("span");
            ico.classList = "icon fiv-viv fiv-icon-folder"
            ...
            let folderContents = generate(tree[i].contents,true);
            folderContents.classList = "folderContents";
            container.appendChild(folderContents);
            fsContainer.appendChild(container);
        }else{
            let container = document.createElement("div");
            container.classList="file";
            let ico = document.createElement("span");
            let extension = tree[i].name.split(".")[1];
            if(filetypes.includes(extension)){
                ico.classList = "icon fiv-viv fiv-icon-" + extension;
...
document.getElementsByTagName("body")[0].appendChild(generate(fstree));
